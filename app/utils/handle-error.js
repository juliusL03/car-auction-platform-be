...
const extractErrMessage = (options, err) => {
	if (err.statusCode && err.statusCode === 403) return 'Login failed'
	if (options.msg) return options.msg
...
const useStatusCodeFromErrorIfAvailable = (options, err) => {
	...
	if (statusCodeFromErr) return statusCodeFromErr
...
const handleError = (error, res, options) => {
	...
	if (options.logging) { console.log(`ERROR: ${errUUID}, ${options.errLocation}, ${errMsg}`) }
	// if printTrace is true and err is an object, print the trace.
	if (options.printTrace && options.logging) console.log('ERROR_TRACE:', error.stack)
